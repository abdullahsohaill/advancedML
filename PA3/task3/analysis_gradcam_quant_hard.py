# analysis_gradcam_cosine.py (Modified for Hard Set Analysis)
...
def main():
    ...
    running specifically on a pre-computed "hard set" of images where the
    baseline fails but the teacher succeeds.
    """
    print("--- Starting Grad-CAM Cosine Similarity Analysis (on Hard Set) ---")
    ...
    # --- 2. Prepare Data Loader (Using the Hard Set) ---
    print("\nLoading the 'Hard Set' for analysis...")
    try:
        hard_set_indices = np.load(os.path.join("results", "hard_set_indices.npy"))
        print(f"Loaded {len(hard_set_indices)} indices for the hard set.")
    except FileNotFoundError:
        print("ERROR: hard_set_indices.npy not found. Please run 'discover_hard_set.py' first.")
        return

    test_images_np = np.load('data/cifar-100-python/test_images.npy')
    
    # Create a subset of the data using these indices
    hard_set_images_np = test_images_np[hard_set_indices]

    # The dataset and loader now only contain the hard images
    hard_dataset = torch.utils.data.TensorDataset(torch.from_numpy(hard_set_images_np))
    test_loader = torch.utils.data.DataLoader(hard_dataset, batch_size=configs.BATCH_SIZE, shuffle=False)
    ...
    pbar = tqdm(test_loader, desc="Calculating CAM Similarities on Hard Set", unit="batch")
    ...
    print("\n--- Localization Knowledge Transfer Analysis (on Hard Set) ---")
    print(f"Analyzed {total_images} 'hard' images.")
