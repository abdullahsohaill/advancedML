# analysis_gradcam_cosine.py (Revised for full model comparison)
...
def main():
    ...
    using the Cosine Similarity comparison method from the paper.
    """
    print("--- Starting Grad-CAM Cosine Similarity Analysis (Full Comparison) ---")
    ...
    test_dataset = torch.utils.data.TensorDataset(torch.from_numpy(test_images_np))
    ...
    pbar = tqdm(test_loader, desc="Calculating CAM Similarities", unit="batch")
    for (images_np,) in pbar:
        for i in range(images_np.shape[0]):
            ...
            # Generate CAMs for Teacher and Independent Student once per image
            ...
            # Now, compare each distilled student against the baseline
    ...
    print("\n--- Localization Knowledge Transfer Analysis ---")
    print(f"Analyzed {total_images} images from the test set.")
    print("Percentage of images where the distilled model's CAM was more similar to the Teacher's than the Baseline's was:")
    ...
    print("\n(A result > 50% indicates successful localization knowledge transfer.)")
