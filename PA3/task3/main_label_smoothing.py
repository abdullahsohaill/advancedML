# main_label_smoothing.py (Updated for Fair Comparison)
...
def main():
    """Main function to run the FAIR Label Smoothing experiment using the torch.hub student."""
    ...
    # --- Setup Logging ---
    experiment_name = "student_hub_label_smoothing"
    ...
    # Load the CIFAR-specialized VGG-11 architecture from the hub, NOT pretrained.
    ...
    # Use standard Cross-Entropy loss but with the label_smoothing parameter enabled
    criterion = nn.CrossEntropyLoss(label_smoothing=configs.LABEL_SMOOTHING_VALUE)
    ...
    print(f"Best Test Accuracy for Hub Student (Label Smoothing): {best_acc:.2f}%")
